...
short device_mpcount[MAX_GPUS] = { 1 };
uint32_t gpus_intensity[MAX_GPUS] = { 16 };
uint32_t device_gpu_clocks[MAX_GPUS] = { 3200 };
uint32_t device_mem_clocks[MAX_GPUS] = { 250 };
...
int device_lookup_gap[MAX_GPUS] = { 1 };
...
int share_result(int result, int pooln, double sharediff, const char *reason)
{
	...
	applog(LOG_NOTICE, "%s%s" CL_N ": [%lu]|" CL_LRD"[%lu]" CL_N " (%s), %s", flag, solved,  p->accepted_count, p->rejected_count, suppl, s);
	if (reason) {
